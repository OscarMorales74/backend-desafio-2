    ...
    async updateProduct(id, updatedFields) {
      ...
      async deleteProduct(id) {
        try {
          ...
          const productIndex = productsFile.findIndex((p) => p.id === id);
          if (productIndex === -1) {
            console.log(`Producto con el id ${id} no se encuentra.`);
            return;
          }
...
const test = async() => {
    ...
    const productToUpdate = { title: 'Motorola', description: 'Nuevo Motorola' };
